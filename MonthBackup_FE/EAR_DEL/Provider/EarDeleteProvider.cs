...
namespace MonthBackup_FE.EAR_DEL.Provider
{
    /// <summary>
    /// EAR 刪除資料提供者：管理全域參數、連線字串、entity 清單
    /// </summary>
    public static class EarDeleteProvider
    {
        // 資料庫連線字串
        ...
        public static bool IsDeleteMode { get; set; } = false; // 預設為匯出模式
        ...
        /// <summary>
        /// 將整數日期轉換為字串格式 (MM/DD/YYYY)
        /// 對應 4GL: let rece_cdate = str_date[5,6], "/", str_date[7,8], "/", str_date[1,4]
        /// </summary>
        public static string ConvertIntToDateString(int dateInt)
        {
            string strDate = dateInt.ToString("D8"); // 確保 8 位數
        ...
        /// <summary>
        /// 檢查是否應跳過特定 entity 和日期的組合
        /// 對應 4GL 中的硬編碼跳過邏輯
        /// </summary>
        public static bool ShouldSkipEntityDate(string entity, int transDate)
        {
            // 20030915 added by hiller for core down
            if (entity == "BL201" && transDate == 20030409) return true;
            if (entity == "BL002" && transDate == 20030410) return true;
            if (entity == "BP002" && transDate == 20030410) return true;
            if (entity == "BB108" && transDate == 20030411) return true;
            if (entity == "BS010" && transDate == 20030412) return true;
